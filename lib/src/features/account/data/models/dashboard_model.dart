class DashboardTransaction {
  ...
  factory DashboardTransaction.fromJson(Map<String, dynamic> json) {
    ...
    int coupons = 1;
    final rawCoupons = json['couponsCount'] ?? json['coupons_count'] ?? json['coupons'] ?? json['couponCount'] ?? json['coupon_count'];
    if (rawCoupons != null) {
    ...
    String? nestedOrderId;
    if (json['order'] is Map) {
      nestedOrderId = json['order']['orderId']?.toString() ?? 
                      json['order']['order_id']?.toString() ??
                      json['order']['orderDisplayRef']?.toString() ?? 
                      json['order']['order_display_ref']?.toString() ??
                      json['order']['id']?.toString();
    }

    final orderDisplayRefVal = nestedOrderId ??
                               json['orderId']?.toString() ??
                               json['order_id']?.toString() ??
                               json['orderDisplayRef']?.toString() ??
                               json['order_display_ref']?.toString() ??
                               json['id']?.toString() ?? '';

    return DashboardTransaction(
      id: orderDisplayRefVal,
      ...
      offer: json['offer']?.toString() ?? json['offerName']?.toString() ?? json['offer_name']?.toString() ?? '',
      code: codeVal,
